...
/**
 * MiniMaxTest is a class that tests if the MiniMax engine finds the best move using
 * scenarios that has already been tested over and over again and we know what the best move is for
 * that specific scenario
 */
class MiniMaxTest {

    /**
     * testFoolsMate() is a test that check if the best move for this scenario is the same as the best move calculated by
    ...
    public void testFoolsMate(){
        ...
        final Move bestMove = Move.MoveFactory.createMove(t3.getBoard(), BoardUtils.getPositionAtTile("d8"), BoardUtils.getPositionAtTile("h4"));
        assertEquals(engineMove, bestMove);
    }

    /**
     * fourMovesCheckMateTest() is a test that check if the best move for this scenario is the same as the best move calculated by
     * the MiniMax algorithm
     */
    @Test
